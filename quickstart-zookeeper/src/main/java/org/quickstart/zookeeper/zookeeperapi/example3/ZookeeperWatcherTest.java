...
/**
 * Zookeeper Watcher 该类实现了org.apache.zookeeper.Watcher类
 * 
 * @author adai
 * @since 2017-09-14
 *
...
public class ZookeeperWatcherTest implements Watcher {
    ...
    /**
     * watcher监控的是客户端与服务器端状态和事件类型 点节点发生改变时，收到来之zookeeper服务器端watch的通知。 (此时客户端相当于一个watcher(监控zookeeper的字节), 实现Watcher类是为了监听服务器的节点数据是否发生变更(监控的动作为watch，客户端可视为watcher.)) 一个客户端可以有多个watcher.
     */
    @Override
    ...
    /**
     * 获取到该节点下的一级子节点,如果needWatch为true，代表监控该节点下的一级节点 (无论该节点是否有子节点，都对该节点的一级子节点进行监控)
     * 
     * @param path 节点全路径
    ...
    public static void main(String[] args) {
        ...
        ZookeeperWatcherTest watcher = new ZookeeperWatcherTest();
        // 连接zookeeper服务器
