 ...
 * 文件名：CuratorZookeeperClient.java
 * 版本信息：
 * 日期：2017年7月2日
...
public class CuratorZookeeperClient {
    ...
    /**
     * key:父路径，如/jobcenter/client/goodscenter value：Map-->key:子路径，如/jobcenter/client/goodscenter/goodscenter00000001 value:路径中的值
     */
    ...
    public String write(String path, String content) throws Exception {
        ...
        String writePath = curator.create().creatingParentsIfNeeded().withMode(CreateMode.EPHEMERAL_SEQUENTIAL).forPath(sb.toString(), content.getBytes("utf-8"));
        return writePath;
    }
            ...
            if (event.getType() == Event.EventType.NodeDataChanged || event.getType() == Event.EventType.NodeCreated) {
                ...
                if (children != null && children.size() > 0) {
                    for (String child : children) {
                        ...
                        byte[] b = curator.getData().usingWatcher(new ZKWatcher(parentPath, childPath)).forPath(childPath);
                        String value = new String(b, "utf-8");
                        if (StringUtils.isNotBlank(value)) {
